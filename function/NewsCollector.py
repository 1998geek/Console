...
class NewsCollector:
    ...
    def collect_news_job(self, rss_url=None, send_email=True):
        rss_url = rss_url or get_setting("RSS_URL")
        if not rss_url:
        ...
        if xml_content:
            ...
            try:
                ...
                if article:
                    ...
                    ad_text = "See https://news.smol.ai/ for the full news breakdowns and give us feedback\n> on @smol_ai!"
    ...
    def send_email(self, title: str, html_content: str, is_use_ssl=True ,is_use_tls=False) -> bool:
        ...
        smtp_user = get_setting("SENDER_USERNAME")
        smtp_pass = get_setting("SENDER_PASSWORD")
        to_addrs_str = get_setting("TO_ADDRS")
        from_alias = get_setting("FROM_ALIAS")
